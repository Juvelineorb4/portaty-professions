...
const GridSearch = ({renderItems, more}) => {
  ...
  if (renderItems.length !== 0) return (
    <View style={{flex: 1,}}>
      ...
      {renderItems.length >= 14 &&<BigRightGrid data={renderItems.slice(13, renderItems.length - 1 < 15 ? renderItems.length - 1 : 16)} value={more + 30}/>}
      {renderItems.length >= 17 &&<LeftGrid data={renderItems.slice(16, renderItems.length - 1 < 20 ? renderItems.length - 1 : 21)} value={more + 35} />}
      {renderItems.length >= 22 &&<RightGrid data={renderItems.slice(21, renderItems.length - 1 < 25 ? renderItems.length - 1 : 26)} value={more + 40}/>}
