...
const List = ({ route, navigation }) => {
  ...
  if (data.length !== 0) return (
    <View style={[{ flex: 1, paddingHorizontal: 10, paddingTop: 40 }, global.bgWhite]}>
      {data.map((post, index) => (
        <ItemProfile key={index} data={post} identityID={user.identityID} styled={{column: styles.columnList}} />
      ))}
    </View>
  );
  if (data.length === 0) return (
    <View style={[{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingBottom: 70 }, global.bgWhite]}>
      <Text style={{fontFamily: 'light', fontSize: 16}}>No tienes ningun negocio registrado</Text>
      <CustomButton
